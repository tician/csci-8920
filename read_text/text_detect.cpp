...
DetectText::~DetectText(){}
	...
	void
DetectText::detect(string filename)
{
	filename_ = filename;
	originalImage_ = imread(filename_);
	if (!originalImage_.data)
	{
		cout << "Cannot read image input...";
		return;
	}
	mode_ = IMAGE;
	detect();
...
DetectText::detect()
{
	...
	cvtColor(originalImage_, imGray, CV_BGR2GRAY);
	...
	time_in_seconds = ((double)getTickCount() - start_time) / (double)getTickFrequency();
...
/* rescale, convert into gray scale */
	void
DetectText::preprocess(Mat& image)
{
	...
	outputPrefix_ = filename_.substr(slashIndex+1,
...
//	bilateralFilter(image, image_, 7, 20, 50);// prosilica sensor noise
	...
	maxStrokeWidth_ = cvRound(20*(float)(max(image.cols,image.rows))/1000);
	...
	void
DetectText::pipeline(int blackWhite)
{
	if (blackWhite == 1)
	{
		fontColor_ = BRIGHT;
...
*/
	...
	void
DetectText::strokeWidthTransform(const Mat& image, Mat& swtmap,
		int searchDirection)
{
	...
	if (firstPass_){
		...
		for(int y = 0; y < edgemap_.rows; y++)
		{
			for(int x = 0; x < edgemap_.cols; x++)
			{
				...
				{
					theta_.at<float>(y,x) =
						atan2(dy.at<float>(y,x), dx.at<float>(y,x));
	...
	updateStrokeWidth(swtmap, edgepoints_, strokePoints,
			searchDirection, UPDATE);

	updateStrokeWidth(swtmap, strokePoints, strokePoints,
			searchDirection, REFINE);

}

	void
DetectText::updateStrokeWidth(Mat& swtmap, vector<Point>& startPoints,
		vector<Point>& strokePoints,
		int searchDirection, Purpose purpose)
{
	...
	for (;itr != startPoints.end(); ++itr)
	{
		...
		{
			float nextY = cvRound(iy + sin(iTheta)*searchDirection*step);
			float nextX = cvRound(ix + cos(iTheta)*searchDirection*step);
		...
		if (isStroke)
		{
			...
			{
				newSwtVal = sqrt((currY - iy)*(currY - iy)
						+ (currX - ix)*(currX - ix));
			}
			...
			{
				swtmap.at<float>(pointStack[i]) =
					min(swtmap.at<float>(pointStack[i]), newSwtVal);
			}
		}
	...
	int
DetectText::connectComponentAnalysis(const Mat& swtmap, Mat& ccmap)
{
	...
	void
DetectText::identifyLetters(const Mat& swtmap, const Mat& ccmap)
{
	...
	for(size_t i = 0 ; i < nComponent_; i++)
	{
		...
		float pixelCount =
			static_cast<float>(iComponentStrokeWidth.size());
		...
		isLetter = isLetter && ((float)sqrt((float)(pow((float)itr->width,(int)2) +
						(float)pow((float)itr->height,(int)2)))
				/maxStrokeWidth < 10);
	...
	void
DetectText::groupLetters(const Mat& swtmap, const Mat& ccmap)
{
	...
	for (size_t i = 0; i < nComponent_; i++)
	{
		...
		float iMeanIntensity = getMeanIntensity(ccmap, iRect,
				static_cast<int>(i));
		float iMedianStrokeWidth = getMedianStrokeWidth(ccmap, swtmap,
				iRect,static_cast<int>(i));
		...
		for (size_t j = i+1; j < nComponent_; j++)
		{
			...
			if (horizontal && vertical)
			{
				if (abs((iRect.x+iRect.width/2)
							-(jRect.x+jRect.width/2)) >=
						abs((iRect.y+iRect.height/2)
			...
			float distance = sqrt((float)pow((float)iRect.x + iRect.width/2 -
						jRect.x - jRect.width/2,(int)2) +
					(float)pow((float)iRect.y + iRect.height/2 -
						jRect.y - jRect.height/2,(int)2));
			int distanceRatio = 4;
			if (horizontal)
			{
				if (distance / max(iRect.width, jRect.width) >
						distanceRatio)
					continue;
			}
			else
			{
				if (distance / max(iRect.height, jRect.height) >
						distanceRatio)
					continue;
			}
			...
			isGroup = isGroup && (max(iMedianStrokeWidth,
						jMedianStrokeWidth)/
					min(iMedianStrokeWidth,
						jMedianStrokeWidth)) < 2;

			// rule 2: height ratio
			isGroup = isGroup && ( max(iRect.height, jRect.height)/
	...
	void
DetectText::chainPairs(Mat& ccmap)
{
	...
	boundingBoxes_.insert(
			boundingBoxes_.end(),
			initialHorizontalBoxes.begin(),
			initialHorizontalBoxes.end()
			);
}

	void
DetectText::chainToBox(vector< vector<int> >& chain,
		vector<Rect>& boundingBox)
{
	...
	bool
DetectText::spaticalOrder(Rect a, Rect b)
{
		return a.y < b.y;
}

	void
DetectText::filterBoundingBoxes(vector<Rect>& boundingBoxes,
		Mat& ccmap, int rejectRatio)
{
	...
	for (size_t i = 0; i < boundingBoxes.size(); i++)
	{
		...
		for (int y = rect->y; y < rect->y + rect->height; y++)
			for (int x = rect->x; x < rect->x + rect->width; x++)
			{
				...
				if (find(components.begin(),components.end(),
							componetIndex) == components.end())
				{
	...
	void
DetectText::overlapBoundingBoxes(vector<Rect>& boundingBoxes)
{
	...
	void
DetectText::overlayText(vector<Rect>& box, vector<string>& text)
{
	double text_scale = 0.5;
	assert(box.size() == text.size());
	Scalar color(0,255,0);
	size_t lineWidth = (size_t) 25/text_scale;
	int indent = 50;
	int count = 1;
	for (size_t i = 0; i < box.size(); i++)
	{
		if (count > 9) indent = 70;
		...
		putText(detection_,
				prefix,
				Point(box[i].x + box[i].width,
					box[i].y + box[i].height),
				FONT_HERSHEY_DUPLEX, 0.75, color, 1);
		putText(detection_,
				prefix,
				Point(image_.cols, textDisplayOffset_*35*text_scale),
				FONT_HERSHEY_DUPLEX, text_scale, color, 1);
		while (output.length()  > lineWidth)
		{
			putText(detection_,
					output.substr(0, lineWidth),
					Point(image_.cols + indent, textDisplayOffset_*35*text_scale),
					FONT_HERSHEY_DUPLEX, text_scale, color, 1);
		...
		const string speakeasy("espeak " + output);
		int r = system(speakeasy.c_str());
	}
}

	void
DetectText::ocrRead(vector<Rect>& boundingBoxes)
{
	sort(boundingBoxes.begin(),
			boundingBoxes.end(),
			DetectText::spaticalOrder);
	for(size_t i = 0; i < boundingBoxes.size(); i++)
	{
		string result;
	...
	float
DetectText::ocrRead(const Mat& imagePatch, string& output)
{
	float score = 0;
	Mat scaledImage;
	...
	int result;
	result = system("tesseract patch.tiff patch");
	assert(!result);
...
// return the score for the input
	float
DetectText::spellCheck(string& str, string& output, int method)
{
...
//		return 9;
	...
	Mat
DetectText::filterPatch(const Mat& patch)
{
	Mat result;
...
/********************* helper functions ***************************/
	void
DetectText::readLetterCorrelation(const char* file)
{
	ifstream fin(file);
...
DetectText::trim(string& str)
{
	...
	void
DetectText::getNearestWord(const string& str, string& nearestWord)
{
	...
	cout << nearestWord << " got the lowest score: "
		<< lowestScore << endl;
}

	void
DetectText::getTopkWords(const string& str, const int k,
		vector<Word>& words)
{
	float score, lowestScore = 100;
	words.clear();
...
// return lowest score in the list
	float
DetectText::insertToList(vector<Word>& words, Word& word)
{
...
DetectText::showEdgeMap()
{
	...
	void
DetectText::showCcmap(Mat& ccmap)
{
	...
	void
DetectText::showSwtmap(Mat& swtmap)
{
	if (firstPass_)
	...
	void
DetectText::showLetterDetection()
{
...
DetectText::showLetterGroup()
{
	...
	void
DetectText::showBoundingBoxes(vector<Rect>& boundingBoxes)
{
	Scalar scalar(0,0,255);
...
void
DetectText::showBoundingBoxes(vector<Rect>& boundingBoxes, vector<bool>& boxInbox)
{
...
inline	int
DetectText::countInnerLetterCandidates(bool* array)
{
	int count = 0;
	for (size_t i = 0; i < nComponent_; i++)
	{
	...
	float
DetectText::getMeanIntensity(const Mat& ccmap, const Rect& rect,
		int element)
{
	assert(element >= 0);
	...
	float
DetectText::getMedianStrokeWidth(const Mat& ccmap, const Mat& swtmap,
		const Rect& rect, int element)
{
	...
	if (componentsMedianStrokeWidth_[element] == 0)
	{
		...
		componentsMedianStrokeWidth_[element] =
			SwtValues[SwtValues.size()/2];
	...
	void
DetectText::mergePairs(const vector<Pair>& groups,
		vector< vector<int> >& chains)
{
	...
	bool
DetectText::mergePairs(const vector< vector<int> >& initialChains,
		vector< vector<int> >& chains)
{
	if(chains.size())
		chains.clear();
				...
				for(size_t kj = 0; kj < initialChains[j].size(); kj++)
				{
					...
					if (initialChains[i][ki] == initialChains[j][kj])
					{
						...
						if (mergedToChainBitMap[j] != -1)
						{
							...
							mergedToChainBitMap[i] =
								mergedToChainBitMap[j];
						}
	...
	void
DetectText::merge(const vector<int>& token, vector<int>& chain)
{
...
// use correlation as indicator of distance
	float
DetectText::editDistanceFont(const string& s, const string& t)
{
	float penalty = 0.7;
		...
		for (int j = 1; j < m+1; j++)
		{
			...
			if ((t[j-1] != sc))
			{
				float correlate = correlation_.at<float>(
						getCorrelationIndex(t[j-1]),
						getCorrelationIndex(sc));
				v = v + 1 - correlate;
			}
...
// get index in correlation matrix for given char
	int
DetectText::getCorrelationIndex(char letter)
{
	if (islower(letter))
	{
		return letter - 'a';
...
// regular editDistance
	int
DetectText::editDistance(const string& s, const string& t)
{
	int n = s.length();
	int m = t.length();
...
/*------------- test functions-------------------*/
	void
DetectText::testGetCorrelationIndex()
{
...
DetectText::testMergePairs()
{
	...
	void
DetectText::testEdgePoints(vector<Point>& edgepoints)
{
